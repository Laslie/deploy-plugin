...
public class Foo {
	...
	public static void main(String[] args) throws Exception {
		JMXServiceURL serviceURL = new JMXServiceURL("t3", "127.0.0.1", 7001,
				"/jndi/weblogic.management.mbeanservers.domainruntime");

		URLClassLoader cl = new URLClassLoader(
				new URL[] { new URL("file:///home/kohsuke/bea/wlserver_10.3/server/lib/weblogic.jar") });
	...
	/**
	 * Trying to access mbeans fail, apparently because CORBA fails to unmarshal
	 * a WL-specific exception class
	 */
	public static void jmain(String[] args) throws Exception {
		...
		System.out
				.println(con.invoke(on, "activate", new Object[] { "src/test/simple.war", "simple", null, null, null },
		...
		// the trick is:
		/*
		 * Setting up IIOP access on monitored machines SiteScope relies on the
		 * IIOP protocol to talk to the WL server. In order to get remote access
		 * with JMX via IIOP you need to provide a default username and password
		 * for the IIOP user.
		 * 
		 * This can be done via the WL admin console as in:
		 * domServiceBus->Environment->Servers->server01_nn->Protocols
		 * [tab]\->IIOP->advanced
		 * 
		 * Then provide a default username and password for the IIOP user.
		 * 
		 * Note: IIOP is already enabled by default, but a username and pwd is
		 * not. This change requires a server restart for WL. You must make sure
		 * that the username and password is >= 8 digits, otherwise the CORBA
		 * connection will fail when using SiteScope.
		 * 
		 * 
		 */
	}
